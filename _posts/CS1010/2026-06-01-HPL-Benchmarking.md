...
[1\. Starting with AI	1](#1.-starting-with-ai)

[1.1. Hardware Reconnaissance	1](#1.1.-hardware-reconnaissance)

[1.2. Software Stack	2](#1.2.-software-stack)

[1.3. Compilation	2](#1.3.-compilation)

[1.4. Tuning & Single Node Baseline	3](#1.4.-tuning-&-single-node-baseline)

[1.5. Multi-Node Scaling and Network Optimization	4](#1.5.-multi-node-scaling-and-network-optimization)

[1.6. Results	4](#1.6.-results)

[2\. Learning	5](#2.-learning)

[2.1. MIT 6.172 (Performance Engineering of Software Systems)	6](#2.1.-mit-6.172-\(performance-engineering-of-software-systems\))

[2.2. SIMD (Single Instruction, Multiple Data) and Vectorization	7](#2.2.-simd-\(single-instruction,-multiple-data\)-and-vectorization)

[2.3. Message Passing	7](#2.3.-message-passing)

[3\. Optimisations	7](#3.-optimisations)

[3.1. OpenBLAS	7](#3.1.-openblas)

[3.2. Scaling it up	8](#3.2.-scaling-it-up)

[3.3. Micro-Tuning (Thread Pinning & Cache-Line Alignment)	8](#3.3.-micro-tuning-\(thread-pinning-&-cache-line-alignment\))

[3.4. The Non-Linearity of Scaled Optimization	9](#3.4.-the-non-linearity-of-scaled-optimization)

[3.5. Motherboard Routing & Topology Micro-Tuning	9](#3.5.-motherboard-routing-&-topology-micro-tuning)

[3.6. MKL	10](#3.6.-mkl)

[3.7. Final Results	11](#3.7.-final-results)

[4\. Conclusions	11](#4.-conclusions)

# **1\. Starting with AI** {#1.-starting-with-ai}
...
## **1.1. Hardware Reconnaissance** {#1.1.-hardware-reconnaissance}
...
**​​Hardware Profile (Single xcn Node):**  
Processors: 2x Intel Xeon E5-2620 v3 (Haswell) @ 2.40 GHz.  
...
**Takeaway & Mathematical Target:**  
The Rpeak for a single node is calculated as: 
> 12 cores \* 2.4 GHz \* 16  FLOPs \= 460.8 GFLOPS

## **1.2. Software Stack** {#1.2.-software-stack}
...
- **Missing Intel MKL:** The cluster lacked an architecture-specific Intel Math Kernel Library (`MKL`). The only viable CPU option was the generic, unoptimized Reference BLAS (`/lib/x86\_64-linux-gnu/libblas.so`)

**Takeaway:**  
The lack of an optimized BLAS library established a severe performance bottleneck from the outset. Reference BLAS relies on generic arithmetic rather than leveraging the CPU's advanced vector instructions (`avx2`)

## **1.3. Compilation** {#1.3.-compilation}
...
2.  Duplicated a generic Linux configuration template (`Make.Linux\_PII\_CBLAS`) into `Make.soc\_cluster`  
...
4.  Executed `make arch=soc\_cluster` to build the `xhpl` binary.

**Takeaway:**   
Code must always be compiled natively on the execution hardware (Linux x86\_64) rather than locally (e.g., on a local machine environment) to prevent immediate architectural incompatibility failures.

## **1.4. Tuning & Single Node Baseline** {#1.4.-tuning-&-single-node-baseline}
...
**Takeaway:**  
The baseline 1-core run achieved \~2.98 GFLOPS (roughly 7.7% of the theoretical peak). This mathematically confirmed the theory from 1.2: utilizing unoptimized Reference BLAS throttles the supercomputer's capability.

## **1.5. Multi-Node Scaling and Network Optimization** {#1.5.-multi-node-scaling-and-network-optimization}
...
- **Interface Name Asymmetry:** Attempting to force the physical device name (`eno0`) failed because interface naming strings varied across the allocated node boards (`xcnc\[1-4\]`). This was bypassed by configuring Open MPI to include resources strictly by IP subnet block: `--mca btl\_tcp\_if\_include 192.168.48.0/21`.  
...
## **1.6. Results** {#1.6.-results}

| Theoretical Peak (Rpeak) | 1,843.20 GFLOPS |  |
| :---- | :---- | :---- |
| This run (Rmax) | 22.27 GFLOPS | 1.21% Efficiency |
...
## **2.1. MIT 6.172 (Performance Engineering of Software Systems)** {#2.1.-mit-6.172-(performance-engineering-of-software-systems)}
...
* Interchange loops  
  * When you interchange your loops, you will get a variation with more cache hits, which saves time  
* Optimisation flags  
  * You can tell clang how much to optimise (-o0, o1, o2, o3)   
* Parallel loops  
  * Use more cores and run loops in parallel with each other  
* Tiling  
  * Manage cache for data reuse, tiled matrix multiplication with parallelism  
* Parallel Divide and Conquer  
  * Tiling for two-level cache, recursive matrix multiplication with parallelism, however when unoptimised, this takes a lot of time due to the overhead of recursion  
  * we can coarsen the recursion to get a much lower running time  
* Compiler vectorisation  
  * Using vector hardware, SIMD with compiler, we can tell the compiler to use the vector instructions specific to the hardware  
* AVX intrinsics  
  * Provides direct access to hardware vector operations
...
[https://people.eecs.berkeley.edu/\~kubitron/cs252/handouts/papers/RooflineVyNoYellow.pdf](https://people.eecs.berkeley.edu/~kubitron/cs252/handouts/papers/RooflineVyNoYellow.pdf)  
[https://www.intel.com/content/www/us/en/docs/advisor/tutorial-roofline/2021-1/overview.html](https://www.intel.com/content/www/us/en/docs/advisor/tutorial-roofline/2021-1/overview.html)  
[https://docs.nersc.gov/tools/performance/roofline/](https://docs.nersc.gov/tools/performance/roofline/)

## **2.2. SIMD (Single Instruction, Multiple Data) and Vectorization** {#2.2.-simd-(single-instruction,-multiple-data)-and-vectorization}
...
[https://youtu.be/YuUMCVX3UVE?si=arV1KhTz0zVrLN1i](https://youtu.be/YuUMCVX3UVE?si=arV1KhTz0zVrLN1i)

## **2.3. Message Passing** {#2.3.-message-passing}
...
[https://hpc-tutorials.llnl.gov/mpi/\#table-of-contents](https://hpc-tutorials.llnl.gov/mpi/#table-of-contents)  
...
**Performance Added:** *Baseline Performance:* **22.27 GFLOPS**

* *Optimised Performance:* **31.70 GFLOPS**  
* *Performance Gain:* **\~42% speedup**

## **3.2. Scaling it up** {#3.2.-scaling-it-up}
...
* **Performance Added:** *Pre-Scaling Performance:* **31.70 GFLOPS**  
  * *Final Peak Performance:* **34.71 GFLOPS**  
  * *Performance Gain:* **9.5% speedup**

## **3.3. Micro-Tuning (Thread Pinning & Cache-Line Alignment)** {#3.3.-micro-tuning-(thread-pinning-&-cache-line-alignment)}

Implemented strict CPU thread affinity binding (`OMP\_PROC\_BIND=true, OMP\_PLACES=cores`) and executed an automated block-size sweep (`NB ∈ {128,192,256}`) at a controlled matrix scale (`N = 30,000`).
...
| :---- | :---- | :---- |
| 128 | 514.10 | 35.01 |
| 192 | Taking forever | I am not waiting for this |
| 256 | zzzz |  |

## **3.4. The Non-Linearity of Scaled Optimization** {#3.4.-the-non-linearity-of-scaled-optimization}
...
**Results:** Peak Micro-Tuned Performance (`N = 30,000`): **35.01 GFLOPS**
...
## **3.5. Motherboard Routing & Topology Micro-Tuning** {#3.5.-motherboard-routing-&-topology-micro-tuning}
